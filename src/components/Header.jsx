...
const Header = () => {
  ...
  return (
    <div
      className={
        "fixed top-0 left-0 w-full z-50 lg:bg-n-8/90 lg:backdrop-blur-sm border-b border-n-6" +
        `${mobileNavOpen ? "bg-n-8" : "bg-n-8/90 backdrop-blur-sm"}`
      }
    >
      <div className={"flex items-center px-5 lg:px-7.5 xl:px-10 max-lg:py-4"}>
        ...
        <Button className={"ml-auto lg:hidden"} px="px-3" onClick={toggleNav}>
          <MenuSvg openNavigation={mobileNavOpen} />
        </Button>
      </div>
    </div>
  );
